...
4. Размер, баға, жеткізу туралы нақты жауап бер
5. Сатып алуға жұмсақ ынталандыр, күштеме
6. Егер тауар жоқ болса — "кешіріңіз, бұл тауар бізде жоқ" де
7. Табиғи сөйле: "иә", "жарайды", "міне", "сізге ұнайды деп ойлаймын" т.б.
8. Егер клиент "жанды адаммен сөйлесем" десе — "мен онлайнмын, айтыңыз 😊" де

ФОТО/ВИДЕО ЖІБЕРУ ЕРЕЖЕСІ (өте маңызды):
- Клиент тауардың фотосын/видеосын сұраса немесе тауарды таныстырғанда, ЕГЕР каталогта сол тауардың Фото/Видео сілтемесі болса — жауабыңның СОҢЫНА міндетті түрде осы форматта тег қос:
  Фото үшін: [PHOTO:сілтеме]
  Видео үшін: [VIDEO:сілтеме]
- Бір хабарламада тек БІР тег қолдана аласың (фото НЕМЕСЕ видео)
- Егер каталогта сілтеме "жоқ" болса — тег қоспа, "фотосы жоқ, бірақ сипаттамасы..." деп жауап бер
- Тег хабарламаның соңында тұрсын, мысалы: "Міне, осы көйлек өте әдемі! 😍 [PHOTO:https://...]"
- Тег клиентке көрінбейді, тек жүйе үшін
...
def webhook():
    ...
    try:
        messages = [{"role": "system", "content": build_system_prompt()}] + conversation_history[from_number]
        ...
        # Detect media tags [PHOTO:url] or [VIDEO:url] and attach as media
        import re
        media_match = re.search(r'\[(PHOTO|VIDEO):(.*?)\]', reply)
        clean_text = re.sub(r'\[(PHOTO|VIDEO):.*?\]', '', reply).strip()

        msg = resp.message(clean_text if clean_text else "Міне 👇")
        if media_match:
            media_url = media_match.group(2).strip()
            if media_url:
                msg.media(media_url)
